...
from typing import List, Union
import requests
from mcp.server.fastmcp import Context  # if needed

@mcp.tool()
def f1_racers(year: Union[int, str]) -> List[List[Union[str, int]]]:
    """
    Get Formula 1 driver standings for a specific year from the Ergast API.

    Args:
        year: The season year (e.g., 2023 or 'current').

    Returns:
        A 2D array suitable for writing to a Google Sheet.
        Each row contains: [Position, Driver, Nationality, Constructor, Points]
    """
    ...
    if response.status_code == 200:
        ...
        rows = [["Position", "Driver", "Nationality", "Constructor", "Points"]]  # header
        ...
        return [["Position", "Driver", "Nationality", "Constructor", "Points"]]  # Header only
...
@mcp.tool()
def create_and_fill_f1_sheet(year: Union[int, str], ctx: Context = None) -> Dict[str, Any]:
    """
    Create a new Google Spreadsheet, write F1 driver standings into it.

    Args:
        year: The Formula 1 season year.

    Returns:
        Dictionary with spreadsheet ID, sheet name, and update result.
    """
    # Step 1: Create Spreadsheet
    sheet_info = create_spreadsheet(f"F1 Standings {year}", ctx=ctx)
    spreadsheet_id = sheet_info["spreadsheetId"]
    sheet_name = sheet_info["sheets"][0]

    # Step 2: Fetch F1 Data
    data = f1_racers(year)

    # Step 3: Write data to spreadsheet
...
@mcp.tool()
def share_existing_sheet(spreadsheet_id: str,
                         email: str,
                         role: str = "writer",
                         ctx: Context = None) -> Dict[str, Any]:
    """
    Share an existing spreadsheet with a user via email.

    Args:
        spreadsheet_id: The ID of the spreadsheet to share.
        email: Email address to share with.
        role: Access role ('reader', 'commenter', or 'writer').

    Returns:
        Sharing result dictionary.
    """
    result = share_spreadsheet(
        spreadsheet_id=spreadsheet_id,
        recipients=[{"email_address": email, "role": role}],
        ctx=ctx
    )
    return result
